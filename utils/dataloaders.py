...
class LoadImagesAndLabels(Dataset):
    ...
    def resize_fire_to_target(self, fire_img, target_size):
        ...
        # 计算目标尺寸与火焰原始尺寸的比例
        scale_ratio_width = min(target_width / fire_width / 4, 1)  # 确保不超过原始宽度的100%
        scale_ratio_height = min(target_height / fire_height / 4, 1)  # 确保不超过原始高度的100%

        # 使用宽度和高度比例中较小的一个，以保持火焰的宽高比
    ...
    def overlay_fire_on_target(self, target_img, fire_img, position, alpha=0.9):
        ...
        # 对于叠加区域，我们需要为每个BGR通道执行操作
        for c in range(3):  # 对BGR每个通道进行处理
            target_region = target_img[y:y + original_h, x:x + original_w, c]
            fire_region = fire_img[:, :, c][by:by + bh, bx:bx + bw]  # 根据实际边界裁剪火焰区域

            # 使用布尔索引更新目标图像的对应区域，确保使用与目标区域匹配的掩码尺寸
    ...
    def change_light(self,img):
        """随机调整亮度"""
        alpha = random.uniform(0.6, 1.4)  # 亮度系数
    ...
    def apply_flame_effect_to_image_and_labels(self,img, labels, img_w, img_h, max_displacement=20):
        ...
        参数:
        - img: 原始图像。
        - labels: 图像的标注，格式为 [class, x_center, y_center, width, height]。
        - img_w: 图像宽度。
        - img_h: 图像高度。
        - max_displacement: 控制点最大位移。

        返回:
        - img_transformed: 变形后的图像。
        - labels_transformed: 更新后的标注。
        """
        # 将YOLO标注转换为像素坐标
    ...
    def load_mosaic(self, index, attempt=1):
        ...
        for i, index in enumerate(indices):
            ...
            # 检查是否仅有一个标签，并且这个标签是火焰（假设火焰的类别标签为0）
        ...
        if not valid:
            if has_fire:
                if attempt < 5:  # 尝试次数小于5次
                    return self.load_mosaic(index, attempt + 1)
                else:
                    # 返回第五次尝试后的结果
                    return img4, labels4
            # # 如果自定义增强标签不完全在图片中，可以选择重新调用load_mosaic或者采取其他策略
    ...
    def label_in_image(self, label, image_shape):
        ...
        :param label: 标签，格式为[class, x_center, y_center, width, height]，其中位置和尺寸是归一化的
        :param image_shape: 图片的形状，格式为(height, width)
        :return: 如果标签在图片内，返回True，否则返回False
        """

...
# class C3(nn.Module):
#     # CSP Bottleneck with 3 convolutions and SEAttention
#     def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
#         super().__init__()
#         c_ = int(c2 * e)  # hidden channels
#         self.cv1 = Conv(c1, c_, 1, 1)
...
#      ECA-C3模块
# class C3(nn.Module):
#     # CSP Bottleneck with 3 convolutions and ECA
#     def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, use_eca=True):
#         super().__init__()
...
#         self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
#         self.eca = EfficientChannelAttention(c2) if use_eca else nn.Identity()
#
#     def forward(self, x):
#         out = self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))
#         return self.eca(out) if self.use_eca else out
